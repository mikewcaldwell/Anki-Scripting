    ...
    templates=[
        {
            ...
            'afmt': '''{{FrontSide}}
                    ...
                    {{/定義（中国語）}}
                    <br>
                    <button id="hintbutton">翻訳を示す</button>
                    <div class='hint' id="grammar" style="display:none">
                        翻訳<br>{{翻訳}}
                    </div>
    ...
    templates=[
        {
            ...
            'afmt': '''{{FrontSide}}<hr id=answer>
                    ...
                    <br>
                    <button id="transbutton">翻訳を示す</button>
                    <div class='hint' id='translation' style='display:none; font-family: Arial; font-size: 20px;'>
                        翻訳<br>{{翻訳}}
                    </div>
                    <br>
                    <button id="grammarbutton">文法を示す</button>
                    <div id="grammar" class='hint' style="display:none">
                        文法<br>{{文法}}
                    </div>
                    <script type="text/javascript" src="_script.jquery-3.5.1.min.js"></script>
                    <script>
...
def import_words(words, deck):
    for word in words:
        #replace the word we are quizzing in each example sentence with __:
        sentences_no_word = map(
            lambda s: s.sentence.replace(word.word, '__'), 
        ...
        aNote = genanki.Note(
            ...
            fields=[
                ...
                word.definition, 
                word.pronunciation, 
                f'[sound:{word.get_audio_file_name()}]',
                sentences_no_word_field, 
                sentences_field
            ]
        )
...
def create_chugokugo_anki_package(media_folder, output_dir, level):
    """
    Creates an anki package for the chugokugo-script.net website 
    based on level passed in. 
    ...
    #To do this need to prefix the file with _script, 
    #'_' just means that Anki won't try to delete the file.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Create an anki deck based on data from Chinese for Japanese speakers website.'
    )
    parser.add_argument(
        'media_folder', 
